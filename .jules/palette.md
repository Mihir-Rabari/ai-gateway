...
## 2026-05-20 - Add screen-reader only labels to inputs relying on placeholders
**Learning:** Using placeholders as the sole identifier for form inputs (like email and password on auth pages) is inaccessible to screen readers.
**Action:** Always include explicitly associated `<label>` elements via `htmlFor` matching the input's `id`. If visually undesirable, apply the `sr-only` class to hide the label while maintaining accessibility.
