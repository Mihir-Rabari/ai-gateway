...
## 2024-05-18 - [UX improvement] Screen reader accessible form labels
**Learning:** Relying solely on placeholders for form inputs is an accessibility anti-pattern. Screen readers require explicit `<label>` elements to properly announce the input's purpose to users.
**Action:** Always pair `Input` components with a corresponding `<label>`. Ensure the label's `htmlFor` attribute matches the input's `id`. If the design dictates a clean UI without visible labels, use the `sr-only` class to hide the label visually while keeping it fully accessible to screen readers.
