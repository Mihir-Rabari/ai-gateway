...
## 2024-05-13 - Add explicit screen reader labels to custom inputs
**Learning:** In `apps/web`, authentication forms rely entirely on visual placeholders for context. Screen readers require explicitly associated `<label>` elements to read out the input's purpose. Even when the design calls for a clean, placeholder-only look, applying the `.sr-only` utility to a `<label>` provides this crucial context to assistive tech without breaking the layout.
**Action:** Always ensure custom text inputs include a `<label>` matching the input's `id`, using the `sr-only` class if visible labels are not intended by the design.
