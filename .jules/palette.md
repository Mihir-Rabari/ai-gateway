...
## 2024-05-24 - [Accessibility] Screen-reader labels for placeholder-only inputs
**Learning:** In frontend components where visual design relies exclusively on placeholders for form inputs (such as the authentication screens in `apps/web`), screen reader accessibility is compromised because placeholders are not robustly announced as form labels by all assistive technologies.
**Action:** When visual design precludes visible labels, always explicitly associate a hidden label with the input by using a `<label htmlFor="[id]" className="sr-only">Label Text</label>` paired with an `id` on the `<Input>`.
