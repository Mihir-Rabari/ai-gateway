...
## 2024-05-01 - Replace raw text toggles with icons
**Learning:** Replaced raw text based toggles ('x', 'menu') with standard SVG icons to improve visual affordance.
**Action:** Always use clear icons from `lucide-react` for interactive elements that were previously just plain text, and ensure keyboard focus indicators alongside an `aria-label` attribute are present.
