...
## 2024-05-06 - [UX improvement] Explicit labels for form accessibility
**Learning:** Auth forms in apps/web were missing explicit `<label>` elements, relying solely on placeholders. This violates screen reader accessibility standards.
**Action:** Always ensure all interactive inputs have explicitly associated `<label>` elements via `htmlFor` and matching `id` attributes. Used `sr-only` to preserve the visual design while making it accessible.
