...
## 2024-05-10 - Missing form labels and loading feedback in authentication forms
**Learning:** Found an accessibility pattern where authentication forms in Next.js web applications (`apps/web`) relied entirely on placeholder text for inputs without explicitly associated `<label>` elements. Furthermore, the submit buttons lacked clear visual loading indicators (e.g., spinners) during async operations.
**Action:** Always ensure all form inputs have associated `<label>` elements (using `htmlFor` and matching `id`s) to maintain screen reader accessibility. When implementing async submit buttons, provide clear visual feedback using a loading spinner like `Loader2` from `lucide-react` to prevent user confusion.
