...
## 2026-05-24 - Screen reader accessible forms via sr-only labels
**Learning:** Form inputs in the `apps/web` application frequently relied entirely on placeholders for visual context, omitting `<label>` elements entirely. This creates significant accessibility barriers for screen readers.
**Action:** Explicitly add `<label>` elements with matching `id` attributes to all form inputs. Use the `sr-only` Tailwind class to visually hide the label while keeping it accessible to screen readers, thus preserving the intended visual design.
