...
## 2024-06-25 - [performance improvement] Single pass array reduce
**Learning:** Using `.filter().reduce()` multiple times over the same transactions array creates intermediate arrays and increases time complexity to O(2N) or O(3N).
**Action:** Combine `.filter().reduce()` operations into a single `.reduce()` pass to prevent intermediate array creation and reduce time complexity to O(N).
