...
## 2025-05-01 - Single-pass Reduce for Array Aggregations
**Learning:** Chaining array methods like `.filter().reduce()` or performing multiple sequential `.reduce()` passes over the same dataset (like transactions) creates unnecessary intermediate arrays and increases time complexity from O(N) to O(2N) or O(3N).
**Action:** Always combine them into a single, unified `.reduce()` operation.
