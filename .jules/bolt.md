...
## 2024-05-13 - [performance improvement] Optimize Array iterations in Console App
**Learning:** Using chained array methods like `.filter().reduce()` or performing multiple sequential `.reduce()` passes over the same dataset creates intermediate arrays and increases time complexity from O(N) to O(2N) or O(3N).
**Action:** Combined multiple array iterations into a single `.reduce()` pass to prevent intermediate array creation and optimize performance, especially for larger datasets.
