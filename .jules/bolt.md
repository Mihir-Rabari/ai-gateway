...
## 2024-06-25 - [performance improvement] Combined chained array methods
**Learning:** Chaining array methods like `.filter().reduce()` or performing multiple sequential `.reduce()` passes over the same dataset increases time complexity from O(N) to O(2N) or O(3N).
**Action:** Combined them into a single, unified `.reduce()` operation to prevent intermediate array creation and improve efficiency.
