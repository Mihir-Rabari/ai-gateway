...
## 2024-05-10 - [performance improvement] Single pass array reductions
**Learning:** Chaining array methods like `.filter().reduce()` or performing multiple sequential `.reduce()` passes over the same dataset creates unnecessary intermediate arrays and increases time complexity to O(2N) or O(3N).
**Action:** Combine them into a single, unified `.reduce()` operation to prevent intermediate array creation and optimize time complexity to O(N).
