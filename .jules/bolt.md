...
## 2026-05-11 - Combine chained array methods to reduce time complexity
**Learning:** Chaining array methods like `.filter().reduce()` or performing multiple `.reduce()` passes over the same dataset creates unnecessary intermediate arrays and increases time complexity from O(N) to O(2N) or O(3N).
**Action:** Combine multiple array operations into a single `.reduce()` pass to optimize performance and memory usage.
