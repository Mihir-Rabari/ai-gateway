...
## 2024-06-25 - Use cursor-not-allowed for disabled states
**Learning:** Using `disabled:pointer-events-none` on interactive elements like buttons suppresses the native tooltip (from the `title` attribute), degrading accessibility and UX for disabled states.
**Action:** Always prefer `disabled:cursor-not-allowed` over `disabled:pointer-events-none` to provide clear visual feedback while allowing native tooltips to display explaining the disabled state.
