...
## 2024-09-20 - Allow native tooltips on disabled buttons
**Learning:** Using `disabled:pointer-events-none` on interactive elements like buttons suppresses native mouse events, preventing the `title` attribute (native tooltip) from appearing and the cursor from changing when hovered by sighted users, degrading accessibility.
**Action:** When implementing disabled states for interactive components, prefer `disabled:cursor-not-allowed` over `disabled:pointer-events-none`.
