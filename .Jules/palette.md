...
## 2024-06-22 - InlineMessage accessibility enhancements
**Learning:** When rendering inline error messages dynamically (e.g., form validation failures in `apps/web` or `danger` toned `InlineMessage` components in `apps/console`), always wrap the text in an element with `role="alert"` and `aria-live="assertive"` (or `status`/`polite` for non-errors) to ensure screen readers immediately announce the notification.
**Action:** When creating or updating components like `InlineMessage`, dynamically assign `role="alert"` and `aria-live="assertive"` for high-priority or destructive notifications (like `danger` or `warning`), and use `role="status"` and `aria-live="polite"` for others (like `success` or `default`).
