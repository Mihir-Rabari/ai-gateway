...
## 2024-09-11 - Replace disabled:pointer-events-none with disabled:cursor-not-allowed
**Learning:** Using `disabled:pointer-events-none` on interactive elements like buttons and inputs suppresses native browser features, such as tooltips provided by the `title` attribute, severely degrading accessibility for sighted users trying to understand why an element is disabled.
**Action:** Always prefer `disabled:cursor-not-allowed` over `disabled:pointer-events-none` for disabled states to maintain pointer interaction capabilities necessary for tooltips while visually indicating the disabled state.
