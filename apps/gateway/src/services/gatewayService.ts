...
export class GatewayService {
  ...
  private async validateToken(token: string, appId?: string): Promise<ValidatedUser> {
    ...
    if (!user) {
      ...
      // Extract clientId before caching to keep hot paths parse-free
      const newParts = token.split('.');
      if (newParts.length === 3) {
        try {
          const payload = JSON.parse(Buffer.from(newParts[1], 'base64url').toString('utf8'));
      ...
      // Fallback for existing cache entries without clientId
      const cachedParts = token.split('.');
      if (cachedParts.length === 3) {
        try {
          const payload = JSON.parse(Buffer.from(cachedParts[1], 'base64url').toString('utf8'));
          if (payload && typeof payload === 'object' && 'clientId' in payload) {
            user.clientId = payload.clientId;
          } else {
            user.clientId = null;
