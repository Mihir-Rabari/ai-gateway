...
export class GatewayService {
  ...
  async processRequest(input: {
    ...
    const currentUsage = await this.clients.redis.eval(RATE_LIMIT_LUA, 1, rateLimitKey, 60);
  ...
  async *processStreamRequest(input: {
    ...
    const rateLimitKey = `ratelimit:gateway:${user.userId}`;
    const currentUsage = await this.clients.redis.eval(RATE_LIMIT_LUA, 1, rateLimitKey, 60);
    if (Number(currentUsage) > limit) throw new GatewayError('RATE_LIMIT_EXCEEDED', 'Rate limit exceeded', 429);
