...
export class GatewayService {
  ...
  async processRequest(input: {
    ...
    // Use an atomic Lua script to prevent race conditions and reduce round-trips
    const script = `
      ...
      return current
    `;
    const currentUsage = await this.clients.redis.eval(script, 1, rateLimitKey) as number;
