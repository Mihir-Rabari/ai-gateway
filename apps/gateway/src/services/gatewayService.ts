...
export class GatewayService {
        ...
        for (const part of parts) {
          ...
          for (const line of part.split('\n')) {
            ...
            if (trimmedLine.startsWith('data: ') && trimmedLine !== 'data: [DONE]') {
              ...
              // PERFORMANCE OPTIMIZATION: Only parse JSON if the payload likely contains usage data.
              // This avoids expensive JSON.parse() on high-volume text generation events.
  ...
  private async validateToken(token: string, appId?: string): Promise<ValidatedUser> {
    ...
    if (!user) {
      ...
      // Fallback for legacy cached entries without clientId
      const parts = token.split('.');
      if (parts.length === 3) {
        try {
          const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8'));
