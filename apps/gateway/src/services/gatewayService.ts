  ...
  private async validateToken(token: string, appId?: string): Promise<ValidatedUser> {
    ...
    if (!user) {
      user = await this.authBreaker.execute(async () => {
        ...
        const validatedUser = json.data;

        // Extract clientId from JWT and store it in the user object
        ...
        if (parts.length === 3) {
          try {
            ...
            if (payload && typeof payload === 'object' && 'clientId' in payload) {
              validatedUser.clientId = payload.clientId;
            }
          } catch (err) {
            // Ignore parse errors, user object just won't have clientId
          }
        }
    ...
    // Verify clientId using the cached value instead of parsing the JWT again
    if (user.clientId) {
      if (!appId || !FIRST_PARTY_APP_IDS.has(appId)) {
