    ...
    <div className="space-y-6">
      ...
      <Surface className="p-6 md:p-7">
        <form onSubmit={handleSubmit} className="space-y-6">
          <Field label="App name" hint="Required" htmlFor="appName"><TextInput id="appName" value={name} onChange={(event) => setName(event.target.value)} placeholder="My AI product" required /></Field>
          <Field label="Description" hint="Optional" htmlFor="appDescription"><TextInput id="appDescription" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Short summary of what the app does" /></Field>
          <Field label="Redirect URIs" hint="One per line" htmlFor="appRedirectUris"><TextArea id="appRedirectUris" value={redirectUrisRaw} onChange={(event) => setRedirectUrisRaw(event.target.value)} placeholder={"http://localhost:3000/callback\nhttps://myapp.com/callback"} /></Field>
