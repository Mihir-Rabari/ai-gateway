...
function TrendChart({ values, labels }: { values: number[]; labels?: string[] }) {
  ...
  // ⚡ Bolt: Avoid O(N) array spread operator overhead and potential call stack size limit errors by using a single reduce pass.
  const max = (values ?? []).reduce((max, val) => (val > max ? val : max), 1);
