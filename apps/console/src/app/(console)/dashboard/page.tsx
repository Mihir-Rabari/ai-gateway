...
function TrendChart({ values, labels }: { values: number[]; labels?: string[] }) {
  ...
  // ⚡ Bolt: Avoid O(N) spread operator overhead and potential call stack limits by using reduce.
...
function MeterList({ items }: { items: Array<{ label: string; value: number; hint?: string }> }) {
  // ⚡ Bolt: Avoid O(N) intermediate array allocation and spread operator overhead by using reduce.
