...
function MeterList({ items }: { items: Array<{ label: string; value: number; hint?: string }> }) {
  // ⚡ Bolt: Replace Math.max(...items.map()) with a single .reduce() pass to avoid O(N) intermediate array allocation overhead, spread operator overhead, and potential "Maximum call stack size exceeded" errors with large datasets.
  const max = (items ?? []).reduce((max, item) => (item.value > max ? item.value : max), 1);
  return (
