...
export default function EarningsPage() {
  ...
  const { debits, credits, estimatedEarnings, pending } = useMemo(() => {
    // ⚡ Bolt: Use a single reduce pass instead of multiple filter().reduce() passes
    // to prevent intermediate array creation and reduce time complexity from O(4N) to O(N).
