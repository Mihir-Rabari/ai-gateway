...
export class RoutingService {
  ...
  private async recordFailure(provider: ProviderName): Promise<void> {
    ...
    // ⚡ Bolt: Use atomic Lua script to prevent un-expiring keys on crash
    const failures = await this.redis.eval(
      `local current = redis.call("INCR", KEYS[1]); redis.call("EXPIRE", KEYS[1], ARGV[1]); return current;`,
      1,
      key,
      300
    ) as number;
