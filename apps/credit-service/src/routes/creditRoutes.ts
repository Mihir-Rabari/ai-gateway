...
export async function creditRoutes(fastify: FastifyInstance) {
  ...
  const authPreHandler: any = async (
    req: FastifyRequest,
    reply: FastifyReply,
  ) => {
    const internalSecret = process.env["INTERNAL_SERVICE_SECRET"];

    // Normalize header which could be string | string[] | undefined
    const rawClientSecret = req.headers["x-internal-secret"];
    const clientSecretStr = Array.isArray(rawClientSecret)
      ? rawClientSecret[0]
      : rawClientSecret;

    if (!internalSecret || !clientSecretStr) {
      return reply
        .status(401)
        .send(
          fail(
    ...
    const clientBuf = Buffer.from(clientSecretStr, "utf8");
  ...
  fastify.post(
    ...
    async (
      req: FastifyRequest<{
        Body: { userId: string; requiredCredits: number };
      }>,
      reply: FastifyReply,
    ) => {
      try {
  ...
  fastify.post(
    ...
    async (
      req: FastifyRequest<{
        Body: { userId: string; requestId: string; amount: number };
      }>,
      reply: FastifyReply,
    ) => {
      try {
        await getService().lock(
          req.body.userId,
          req.body.requestId,
          req.body.amount,
        );
  ...
  fastify.post(
    ...
    async (
      req: FastifyRequest<{ Body: { userId: string; requestId: string } }>,
      reply: FastifyReply,
    ) => {
      try {
  ...
  fastify.post(
    ...
    async (
      ...
      try {
        const result = await getService().addCredits(
          req.body.userId,
          req.body.amount,
          req.body.reason,
        );
  ...
  fastify.get(
    ...
    async (
      req: FastifyRequest<{
        Querystring: { userId: string; limit: number; offset: number };
      }>,
      reply: FastifyReply,
    ) => {
      try {
        const { userId, limit, offset } = req.query;
        const transactions = await getService().getTransactions(
          userId,
          limit,
          offset,
        );
