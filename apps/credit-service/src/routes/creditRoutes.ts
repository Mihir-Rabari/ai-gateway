...
export async function creditRoutes(fastify: FastifyInstance) {
  ...
  const authPreHandler: any = async (
    ...
    if (!internalSecret || !clientSecret || typeof clientSecret !== "string") {
      return reply
        .status(401)
        .send(
          fail(
    ...
    if (
      internalSecretBuf.length !== clientSecretBuf.length ||
      !timingSafeEqual(internalSecretBuf, clientSecretBuf)
    ) {
      return reply
        .status(401)
        .send(
