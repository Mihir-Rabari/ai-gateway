...
export class AIGateway {
  ...
  private decodeTokenExpiry(token: string): number | null {
    ...
    try {
      ...
      if (parts.length === 3) {
        let json: string;
        // ⚡ Bolt: Fast-path for Node.js using native Buffer decoding.
  ...
  private base64urlEncode(str: string): string {
    // ⚡ Bolt: Fast-path for Node.js using native Buffer encoding.
    if (typeof Buffer !== 'undefined') {
      return Buffer.from(str, 'utf8').toString('base64url');
    }
    const bytes = new TextEncoder().encode(str);
    let binary = '';
    // ⚡ Bolt: Chunked array processing to avoid O(N^2) string concatenation overhead in browsers.
