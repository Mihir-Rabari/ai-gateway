...
export const errorHandlerPlugin = fp(async (fastify: FastifyInstance) => {
  fastify.setErrorHandler((error, req, reply) => {
    ...
    // Mask the original error message for 5xx errors to prevent info leakage
    const safeMessage = statusCode >= 500 ? 'Internal server error' : (appError.message ?? 'Internal server error');
