...
Never leave a deserving PR open. If a PR is blocked only by a merge conflict, that is
fixable — fix it (via @jules) and come back to it.
Skip undeserving PRs gracefully. If a PR has unresolved review comments, failing CI,
has been marked as draft, or is flagged WIP — skip and leave a comment explaining why.
Delegate rebases to @jules. Never attempt manual conflict resolution directly. Always
assign rebase tasks to @jules via a structured GitHub comment, then poll for completion.
Work in passes. First pass: merge all clean PRs. Second pass: return to conflicting
...
Use the GitHub API (or gh CLI) to list all open PRs in ai-gateway:

bash   gh pr list --repo <owner>/ai-gateway --state open --json number,title,mergeable,reviews,statusCheckRollup,isDraft,labels

For each PR, classify it into one of these buckets:
BucketCriteria✅ MERGE NOWMergeable, CI passing, approved (or no review required), not draft⚠️ CONFLICTINGmergeable: CONFLICTING — needs rebase⏭️ SKIPDraft, WIP label, failing CI with no fix path, unresolved review threads🔍 NEEDS REVIEWOpen, CI passing, but no approval yet — leave a comment asking for review if appropriate

Log the triage result for all PRs before taking any action.


PHASE 2 — Merge Clean PRs (First Pass)
For each PR in the MERGE NOW bucket:

Do a final check: confirm CI is green and no new conflicts appeared.
Merge using squash or merge commit per the repo's convention:

bash   gh pr merge <PR_NUMBER> --repo <owner>/ai-gateway --squash --auto

After merge, confirm success and log: ✅ Merged PR #<number>: <title>.
Move to next PR.
...
Has @jules left a comment indicating the rebase is done?



bash     gh pr view <PR_NUMBER> --repo <owner>/ai-gateway --comments | grep -i "rebase complete\|rebased\|done\|finished"

Is the PR now mergeable: MERGEABLE?

bash     gh pr view <PR_NUMBER> --repo <owner>/ai-gateway --json mergeable
...
Re-run CI check (wait for green if it just started).
Merge:

bash   gh pr merge <PR_NUMBER> --repo <owner>/ai-gateway --squash --auto

Log: ✅ Merged PR #<number> after rebase by @jules.


PHASE 4 — Final Sweep & Report
After all PRs have been processed:

List any remaining open PRs and state why they were not merged (skip reason).
...
If any PRs were delegated to @jules and are still pending, schedule a follow-up check
...
Tools to Use
TaskToolList PRsgh pr list or GitHub REST APICheck mergeabilitygh pr view --json mergeablePost commentgh pr comment <number> --body "..."Merge PRgh pr merge <number> --squashCheck @jules activitygh pr view <number> --commentsCheck CI statusgh pr checks <number>
